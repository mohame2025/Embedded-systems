...
#define PWM_INTERFACE_H
	...
	typedef struct{
		uint8_t WGM		: 1;							// Wave form generation mode
		uint8_t COM		: 2;							// Compare output mode
		uint8_t  CS		: 3;							// Prescaler value
		uint8_t  Timer	: 2;							// Timer selection
	...
	uint8_t gen_PWM(PWM_config* );
	// Set callback function in case of normal COM
	void set_callback(uint8_t copy_vector_id, void (*call_back_function)(void) );
	// Stop pulse width modulation for certain timer
	uint8_t PWM_stop(uint8_t );
