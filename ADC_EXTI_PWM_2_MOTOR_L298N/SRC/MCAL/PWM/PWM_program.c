...
uint8_t gen_PWM(PWM_config* copy_ptr_PWM_config){
	/*
	 *
	 */
	if((copy_ptr_PWM_config -> Timer) == PWM_TIMER0){
		...
		PWM_TCNT0 = *((uint8_t*)(copy_ptr_PWM_config -> preload));
		// Load the value of the OCR register
		PWM_OCR0 = *((uint8_t*)(copy_ptr_PWM_config -> preload)+1);
		...
		// Load the value of the TCNT register
		PWM_TCNT2 = *((uint8_t*)(copy_ptr_PWM_config -> preload));
		// Load the value of the OCR register
		PWM_OCR2 = *((uint8_t*)(copy_ptr_PWM_config -> preload)+1);
		// in normal mode interrupts must be enabled
		if((copy_ptr_PWM_config -> COM) == PWM_normal){
...
uint8_t PWM_stop(uint8_t copr_timer_id){
	/*
	 *
	 */
...
void set_callback(uint8_t copy_vector_id, void (*copy_ptr_call_back_function)(void) ){
	/*
	 *
	 */
